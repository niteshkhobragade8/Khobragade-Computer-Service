...
import {professionalCategory} from './service-categories.js';
...
function serviceCategory(id){const s=window.__v30Services?.find(x=>x.id===id);return professionalCategory(s?.category,s?.name)}
function renderDocManager(){
 const sel=$('v30DocsAction'); if(!sel)return;
 const current=sel.value; const cat=$('formCategoryFilter')?.value||'all';
 sel.innerHTML=actions.filter(a=>cat==='all'||serviceCategory(a.serviceId)===cat).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''))).map(a=>`<option value="${esc(a.id)}">${esc(serviceName(a.serviceId))} → ${esc(a.name||'Action')}</option>`).join('');
...
$('formCategoryFilter')?.addEventListener('change',renderDocManager);
