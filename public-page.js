...
import {collection,onSnapshot,doc,setDoc,increment,serverTimestamp} from './supabase-compat.js';
...
function base(kind){if(settings.catalogMode && settings.catalogMode!=='bundled')return[];return kind==='yojana'?DEFAULT_SCHEMES:kind==='divyang'?DEFAULT_DIVYANG:DEFAULT_SERVICES}
...
function searchIndex(){let out=[];mergeUnique(DEFAULT_SERVICES,services).forEach(x=>out.push({name:x.name,meta:x.category||'Service',url:'services.html?q='+encodeURIComponent(x.name),text:`${x.name} ${x.category} ${x.description}`}));mergeUnique(DEFAULT_SCHEMES,relevant('yojana')).forEach(x=>out.push({name:x.name,meta:'Government Yojana',url:'maharashtra.html?q='+encodeURIComponent(x.name),text:`${x.name} ${x.category} ${x.description}`}));mergeUnique(DEFAULT_DIVYANG,relevant('divyang')).forEach(x=>out.push({name:x.name,meta:'Divyang',url:'divyang.html?q='+encodeURIComponent(x.name),text:`${x.name} ${x.category} ${x.description}`}));((settings.documentsMode && settings.documentsMode!=='bundled')?editableChecklists.filter(x=>(x.status||'Published')==='Published'):DOCUMENT_CHECKLISTS).forEach(x=>out.push({name:x.title+' Documents',meta:'Documents',url:'documents.html?q='+encodeURIComponent(x.title),text:`${x.title} ${x.category} ${(x.items||[]).join(' ')}`}));return out}
