...
let settings={},services=[],updates=[],docs=[],notifications=[],pages={},youtubeVideos=[],cmsGlobal={},cmsHome={},siteSections=[],editableChecklists=[],dynamicPages=[];
...
function renderDynamicMenu(){const h=$('dynamicMenuItems');if(!h)return;h.innerHTML=dynamicPages.filter(x=>(x.status||'Published')==='Published'&&x.inMenu!==false).sort((a,b)=>Number(a.order||0)-Number(b.order||0)).map(x=>`<a class="dynamic-nav-link" href="dynamic-page.html?page=${encodeURIComponent(x.slug||'')}">${esc(x.name||'Page')}</a>`).join('')}
