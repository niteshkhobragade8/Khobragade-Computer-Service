...
let settings={},services=[],updates=[],docs=[],notifications=[],pages={},youtubeVideos=[],cmsGlobal={},cmsHome={},siteSections=[],editableChecklists=[],dynamicPages=[],extraMenus=[],activeTheme={};
...
function renderPopup(){const x=notifications.filter(n=>(n.status||'Published')==='Published'&&n.type==='popup').sort((a,b)=>time(b.createdAt||b.updatedAt)-time(a.createdAt||a.updatedAt))[0];if(!x||sessionStorage.getItem('popup-'+x.id)==='seen')return;sessionStorage.setItem('popup-'+x.id,'seen');const o=document.createElement('div');o.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:99999;display:grid;place-items:center;padding:20px';o.innerHTML=`<div style="width:min(480px,100%);background:#fff;border-radius:20px;padding:24px;box-shadow:0 25px 70px rgba(0,0,0,.3)"><div style="display:flex;justify-content:space-between;gap:12px"><h3>${esc(x.title)}</h3><button data-close-popup style="border:0;background:#eee;border-radius:50%;width:34px;height:34px">✕</button></div><p style="margin-top:10px">${esc(x.description||'')}</p></div>`;o.querySelector('[data-close-popup]').onclick=()=>o.remove();o.onclick=e=>{if(e.target===o)o.remove()};document.body.appendChild(o)}
...
function searchIndex(){let out=[];mergeUnique(DEFAULT_SERVICES,services).forEach(x=>out.push({name:x.name,meta:x.category||'Service',url:'services.html?q='+encodeURIComponent(x.name),text:`${x.name} ${x.category} ${x.description}`}));mergeUnique(DEFAULT_SCHEMES,relevant('yojana')).forEach(x=>out.push({name:x.name,meta:'Government Yojana',url:'maharashtra.html?q='+encodeURIComponent(x.name),text:`${x.name} ${x.category} ${x.description}`}));mergeUnique(DEFAULT_DIVYANG,relevant('divyang')).forEach(x=>out.push({name:x.name,meta:'Divyang',url:'divyang.html?q='+encodeURIComponent(x.name),text:`${x.name} ${x.category} ${x.description}`}));(settings.documentsMode==='firebase'?editableChecklists.filter(x=>(x.status||'Published')==='Published'):DOCUMENT_CHECKLISTS).forEach(x=>out.push({name:x.title+' Documents',meta:'Documents',url:'documents.html?q='+encodeURIComponent(x.title),text:`${x.title} ${x.category} ${(x.items||[]).join(' ')}`}));return out}
...
applySettings();applyTheme();applyLanguage();applyFullCms();renderServices();renderFeatured();renderDocs();renderYoutube();applyPage();
onSnapshot(doc(db,'settings','website'),s=>{settings=s.exists()?s.data():{};if(!currentLang)currentLang=settings.defaultLanguage||'en';applySettings();applyTheme();applyLanguage();applyFullCms();renderServices();renderFeatured();renderDocs();renderYoutube()});
