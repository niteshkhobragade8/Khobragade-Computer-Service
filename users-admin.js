...
function render(){
 ...
 const normalUsers=users.filter(u=>!isCommissionUser(u));
 const rows=normalUsers.filter(u=>!q||[u.fullName,u.mobile,u.email].join(' ').toLowerCase().includes(q));
 const totalEl=$('totalUsers');if(totalEl)totalEl.textContent=String(normalUsers.length);
