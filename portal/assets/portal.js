...
async function services(){
 ...
 serviceMasterMap.clear();serviceLiveMap.clear();
 allLive.forEach(x=>serviceLiveMap.set(x.id,x));
 ...
 // V25: always merge bundled master catalogue with live admin services so no configured service disappears.
 ...
 rows.forEach(x=>serviceLiveMap.set(x.id,x));
 return rows;
}
async function actions(serviceId){
 ...
 if(!live.length){const sr=serviceLiveMap.get(serviceId)||{id:serviceId,name:'Service'};const id=('auto_'+serviceId).replace(/[^a-zA-Z0-9_-]/g,'_').slice(0,120);const charge=Number(sr.serviceCharge??sr.charge??sr.price??0);const row={id,serviceId,name:'Online Application Assistance',serviceCharge:Number.isFinite(charge)?charge:0,officialFee:0,description:`${sr.name||'Service'} online application / form assistance.`,requiredDocuments:['Aadhaar Card','Photo','Signature','Supporting Document (if applicable)'],availabilityStatus:'Available',available:true,order:10,autoGenerated:true};live.push(row);actionMasterMap.set(id,{generic:true,service:sr})}
...
async function fields(actionDocId){
 ...
 const mapped=actionMasterMap.get(actionDocId);if(mapped?.generic){const defs=[{label:'Full Name',type:'text',required:true,profileKey:'fullName'},{label:'Date of Birth',type:'date',required:true,profileKey:'dob'},{label:'Gender',type:'select',required:false,profileKey:'gender',options:['Male','Female','Other']},{label:'Mobile Number',type:'tel',required:true,profileKey:'mobile'},{label:'Full Address',type:'textarea',required:true,profileKey:'address'},{label:'Village / City',type:'text',required:false,profileKey:'villageCity'},{label:'Taluka',type:'text',required:false,profileKey:'taluka'},{label:'District',type:'text',required:false,profileKey:'district'},{label:'State',type:'text',required:false,profileKey:'state'},{label:'PIN Code',type:'text',required:false,profileKey:'pinCode'},{label:'Application / Form Name',type:'text',required:false},{label:'Additional Details',type:'textarea',required:false}];return defs.map((f,i)=>({id:`auto_f_${actionDocId}_${i+1}`,...f,order:(i+1)*10,actionId:actionDocId}))}if(mapped)return fieldsFor(mapped.action).map((f,i)=>({id:`fld_${actionDocId}_${i+1}`,...f,actionId:actionDocId}));
...
function memberChrome(profileData,cms={}){
 const file=(location.pathname.split('/').pop()||'index.html').toLowerCase();let meta=memberPages[file];const slug=new URLSearchParams(location.search).get('slug')||'';if(file==='custom-page.html'){const pg=(cms.pages||[]).find(x=>x.slug===slug&&x.visible!==false);meta=pg?{title:pg.title||pg.heading||'Page',key:'custom:'+slug}:null}if(!meta||!currentUser)return;const pageCfg=(cms.pages||[]).find(x=>x.file===file||x.key===meta.key||x.slug===slug);if(pageCfg?.visible===false){location.href='account.html';return}if(pageCfg){meta={...meta,title:pageCfg.title||pageCfg.heading||meta.title};document.title=(pageCfg.title||pageCfg.heading||meta.title)+' - KCSC Portal';queueMicrotask(()=>{const main=document.querySelector('main');const h=main?.querySelector('h1,h2');const p=main?.querySelector('.section-head p,.page-intro p,.dashboard-subtitle');if(h&&pageCfg.heading)h.textContent=pageCfg.heading;if(p&&pageCfg.intro)p.textContent=pageCfg.intro})}
