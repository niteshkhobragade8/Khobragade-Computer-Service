...
async function startPayment(app){
 if(app.amount<=0)return {free:true};
 const target='payment.html?id='+encodeURIComponent(app.docId||app.applicationId);

 // GitHub Pages can occasionally return its temporary Unicorn error on the
 // first request. Check the target first and retry briefly before navigation.
 const wait=ms=>new Promise(r=>setTimeout(r,ms));
 for(let attempt=1;attempt<=4;attempt++){
  try{
   const r=await fetch(target+(target.includes('?')?'&':'?')+'_ready='+Date.now(),{
    method:'GET',
    cache:'no-store',
   ...
   if(r.ok && type.includes('text/html')){
    ...
    return {redirected:true,attempt};
   }
  }catch(_){}
  await wait(attempt*450);
 }

 // Final fallback: navigate normally. This preserves the existing flow even
 // if the availability probe itself was blocked.
 location.href=target;
...
function memberChrome(profileData,cms={}){
 ...
 const baseMenu=(Array.isArray(cms.memberMenu)&&cms.memberMenu.length?cms.memberMenu:DEFAULT_MEMBER_MENU).map(x=>{
   if(x.key==='help')return {...x,href:'index.html#support'};
