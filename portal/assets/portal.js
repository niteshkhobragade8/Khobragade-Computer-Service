...
import {MASTER_SERVICES,actionId,fieldsFor} from './master-catalog.js?v=20260821-v25menupages';
...
function memberChrome(profileData,cms={}){
 const file=(location.pathname.split('/').pop()||'index.html').toLowerCase();let meta=memberPages[file];if(file==='page.html'){const slug=new URLSearchParams(location.search).get('slug')||'';const p=(cms.customPages||[]).find(x=>x.slug===slug&&x.deleted!==true&&x.visible!==false);if(p)meta={title:p.title||p.name||'Page',key:'page_'+slug};}if(!meta||!currentUser)return;const pageOverride=cms.pageOverrides?.[meta.key]||{};meta={...meta,title:pageOverride.title||meta.title};
 ...
 if(file!=='page.html'&&pageOverride.content){const host=document.querySelector('.dashboard-content')||document.querySelector('main .container')||document.querySelector('main');if(host&&!document.getElementById('portalCmsPageIntro')){const box=document.createElement('section');box.id='portalCmsPageIntro';box.className='portal-cms-page-intro';box.innerHTML=`<h2>${esc(pageOverride.title||meta.title)}</h2><p>${esc(pageOverride.content).replace(/\n/g,'<br>')}</p>`;host.prepend(box)}}
